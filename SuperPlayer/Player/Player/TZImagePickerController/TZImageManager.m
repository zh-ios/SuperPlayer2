    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success && completion && localIdentifier) {
                ...
                if (error) {
                    NSLog(kZHLocalizedString(@"保存照片出错:%@"),error.localizedDescription);
                }
                if (completion) {
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success && completion && localIdentifier) {
                ...
                if (error) {
                    NSLog(kZHLocalizedString(@"保存视频出错:%@"),error.localizedDescription);
                }
                if (completion) {
                    completion(nil, error);
                }
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset timeRange:(CMTimeRange)timeRange presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure  {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([supportedTypeArray containsObject:AVFileTypeMPEG4]) {
            ...
            if (failure) {
                failure(kZHLocalizedString(@"该视频类型暂不支持导出"), nil);
            }
            NSLog(kZHLocalizedString(@"No supported file types 视频类型暂不支持导出"));
            return;
        } else {
        ...
        if (failure) {
            NSString *errorMessage = [NSString stringWithFormat:kZHLocalizedString(@"当前设备不支持该预设:%@"), presetName];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        switch (session.status) {
            ...
            case AVAssetExportSessionStatusFailed: {
                ...
                if (failure) {
                    failure(kZHLocalizedString(@"视频导出失败"), session.error);
                }
            }  break;
            case AVAssetExportSessionStatusCancelled: {
                NSLog(@"AVAssetExportSessionStatusCancelled");
                if (failure) {
                    failure(kZHLocalizedString(@"导出任务已被取消"), nil);
                }
            }  break;
            default: break;
        }
