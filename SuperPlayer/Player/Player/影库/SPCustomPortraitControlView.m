...
@interface SPCustomPortraitControlView () <ZFSliderViewDelegate>
/// 底部工具栏
...
@implementation SPCustomPortraitControlView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.topToolView addSubview:self.downloadButton];
...
- (void)addCloseButton {
    ...
    self.closeButton = backBtn;
    [backBtn setImage:kResizedImage(@"sp_icon_close_white", 22) forState:UIControlStateNormal];
...
- (void)layoutSubviews {
    ...
    self.downloadButton.frame = CGRectMake(kScreenWidth - 45, kTopSafeArea+10, 30, 30);
...
- (UIButton *)downloadButton {
    if (!_downloadButton) {
        ...
        _downloadButton.extendedHitArea = CGRectMake(20, 20, 20, 20);
    }
    return _downloadButton;
}

- (void)downloadCurrentVideo {
    if (self.downloadBtnClickCallbackBlock) {
        self.downloadBtnClickCallbackBlock();
    }
}
