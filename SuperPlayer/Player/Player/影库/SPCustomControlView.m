...
#import "SPCustomPortraitControlView.h"
#import "SPCustomLandscapeControlView.h"
...
@interface SPCustomControlView () <ZFSliderViewDelegate>
/// 竖屏控制层的View
@property (nonatomic, strong) SPCustomPortraitControlView *portraitControlView;
/// 横屏控制层的View
@property (nonatomic, strong) SPCustomLandscapeControlView *landScapeControlView;
/// 加载loading
...
// 倍速view
@property (nonatomic, strong) SPMultiSpeedView *rateView;

@end

@implementation SPCustomControlView
@synthesize player = _player;


#define kRateViewH          (135)
#define kRateViewW          (40)
#define kRateViewTop        (kNavbarHeight+10)
...
- (void)layoutSubviews {
    ...
    self.rateView.frame = CGRectMake(kScreenWidth-52, kRateViewTop, kRateViewW, kRateViewH);
}
...
- (void)showRateView {
    ...
    [self.rateView updateUIWithCurrentRate:self.player.currentPlayerManager.rate*10];
    if (self.player.isFullScreen) {
        self.rateView.alpha = 0.0;
        [UIView animateWithDuration: self.autoFadeTimeInterval  animations:^{
            self.rateView.alpha = 1.f;
        } completion:^(BOOL finished) {
...
- (SPCustomPortraitControlView *)portraitControlView {
    if (!_portraitControlView) {
        @zf_weakify(self)
        _portraitControlView = [[SPCustomPortraitControlView alloc] init];
        ...
        _portraitControlView.downloadBtnClickCallbackBlock = ^{
            @zf_strongify(self)
...
- (SPCustomLandscapeControlView *)landScapeControlView {
    if (!_landScapeControlView) {
        ...
        _landScapeControlView.downloadBtnClickCallbackBlock = ^{
            @zf_strongify(self)
            if (self.downloadBtnClickCallback) {
                self.downloadBtnClickCallback();
            }
