...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self initSubviews];
    self.title = kZHLocalizedString(@"在线播放");
...
- (void)didBecomeActive:(NSNotification *)noti {
    ...
    if (pasteBoard.hasStrings) {
        NSDictionary *dict = [pasteBoard.items firstObject];
//        @"public.url"
        NSString *pasteBoardString = dict[@"public.utf8-plain-text"];
        if (![pasteBoardString isKindOfClass:[NSString class]] || !pasteBoardString) return;
        BOOL hasLinks = ([[pasteBoardString uppercaseString] hasPrefix:@"MAGNET"] || [[pasteBoardString uppercaseString] hasPrefix:@"HTTP"]);
        if (hasLinks) {
            [ZHToastUtil showToast:kZHLocalizedString(@"检测到链接，已自动为您粘贴")];
            self.urlView.text = pasteBoardString;
        }
    }
}

- (void)initSubviews {
    UILabel *line = [[BaseLabel alloc] initWithFrame:CGRectMake(15, kNavbarHeight+50, self.view.width-15*2, 5)];
    ...
    UILabel *label = [[BaseLabel alloc] initWithFrame:shim.bounds];
